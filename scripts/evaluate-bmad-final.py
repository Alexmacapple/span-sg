...
def score_story(file_path: Path) -> dict:
    ...
    has_acceptance = bool(
        re.search(r'##\s+Critères\s+d[\'"]acceptation', content, re.I)
    )
    scores["structure"] = (
        has_context + has_objective + has_prereqs + has_steps + has_acceptance
    ) * 5
    ...
    has_error_section = bool(
        re.search(
            r'##\s+(Messages\s+d[\'"]erreur|Problèmes.*fréquents|Dépannage)',
            content,
            re.I,
        )
    )
    ...
    return {**scores, "total": total, "grade": get_grade(total)}
...
def main():
    ...
    print("=" * 100)
    print(
        f"{'Story':<25} {'Struct':<8} {'Cmd':<6} {'Test':<6} {'Err':<6} {'Deps':<6} {'Auto':<6} {'Refs':<6} {'Risk':<6} {'Total':<8} {'Grade':<6}"
    )
    print("=" * 100)
    ...
    if avg_total >= 99.9:
        ...
        print(
            f"🎯 OBJECTIF PRESQUE ATTEINT : {avg_total:.1f}/100 ({improvement:+.1f} points)"
        )
        print(f"   Manque {100 - avg_total:.1f} points pour perfection")
    else:
        print(
            f"⚠️  Objectif non atteint : {avg_total:.1f}/100 ({improvement:+.1f} points)"
        )
